...
public class MyRemindsPresenter implements Presenter {

  private static final String TAG = MyRemindsPresenter.class.getSimpleName();
  ...
  @NonNull
  private MyRemindsView mView;

  public MyRemindsPresenter(@NonNull MyRemindsView view) {
    mView = view;
  }
  ...
  public void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    final Timestamp lastWeek = new Timestamp(DateTime.now().minusWeeks(1).toDate());
    mUser = FirebaseAuth.getInstance().getCurrentUser();
    mFirestore = FirebaseFirestore.getInstance();
    mQuery = mFirestore.collection("reminds")
        .whereEqualTo("authorId", mUser.getUid())
        .whereGreaterThan("due", lastWeek)
        .orderBy("due", Direction.ASCENDING);
  }
  ...
  public void onRemindViewHolderClicked(@NonNull String thingId) {
